...
const topPointsRanking = () => {
	const { apiURL } = useContextProvider();
	const [data, setData] = useState<topData[]>([]);
	const [isLoading, setIsLoading] = useState(true);

	useEffect(() => {
		console.log(`API GET: ${apiURL}/dashboard/top-points/10: Inicio llamada a las ${getHour()}`)
		getData();
		console.log(`API GET: ${apiURL}/dashboard/top-points/10: Respuesta obtenida a las ${getHour()}`)
	}, []);

	const getData = async () => {
		setIsLoading(true);
		await fetch(apiURL + '/dashboard/top-points/10')
		...
		<View style={styles.container}>
			...
			{!isLoading && <View style={styles.dataContainer}>
				<View style={[styles.title, styles.rowUser]}>
					...
					<View style={styles.total}><Text style={styles.titleText}>Puntos</Text></View>
				</View>
				{data.map((item, index) => {
					return (
						<View style={styles.rowUser} key={index}>
							...
							<View style={styles.total}>
								<Text style={styles.text}>{item.totPoints}</Text>
							</View>
						</View>
					)
				})}
